...
describe('inspect', () => {
  it('should return boxed result values', async () => {
    const result = await inspect('5 + 3');
  ...
  it('should analyze AST complexity', async () => {
    const result = await inspect('Patient.name.where(use = "official").given');
  ...
  it('should track operation timings', async () => {
    const result = await inspect('5 + 3 * 2');
  ...
  it('should generate hints for inefficient patterns', async () => {
    const result = await inspect('items.where(active = true).where(type = "foo")');
  ...
  it('should not collect traces by default', async () => {
    const result = await inspect('5.trace("five") + 3');
  ...
  it('should collect traces when enabled', async () => {
    ...
    const result = await inspect('5.trace("five") + 3.trace("three")', {
      includeTraces: true
    });
  ...
  it('should work with input data', async () => {
    const result = await inspect('name.given', {
  ...
  it('should work with variables', async () => {
    const result = await inspect('%x + %y', {
      variables: { x: 10, y: 20 }
    });
  ...
  it('should respect maxDepth for AST analysis', async () => {
    const deepExpression = 'a.b.c.d.e.f.g.h.i.j.k.l.m.n.o.p';
    const result = await inspect(deepExpression, { maxDepth: 5 });
    
    expect(result.ast.metadata.depth).toBeLessThanOrEqual(5);
  });
  
  it('should generate hint for count() > 0 pattern', async () => {
    // Note: This pattern detection requires parent tracking in AST
    // which is not currently implemented
    const result = await inspect('items.count() > 0');
  ...
  it('should generate hint for unnecessary empty() before first()', async () => {
    const result = await inspect('items.empty().first()');
  ...
  it('should provide detailed operation counts', async () => {
    const result = await inspect('(5 + 3) * 2 - 1');
  ...
  it('should track function operation timings', async () => {
    const result = await inspect('items.where(active = true).count()');
