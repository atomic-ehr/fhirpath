...
describe('FHIRPath Parser', () => {
  ...
  describe('Functions', () => {
    ...
    it('should parse method syntax', () => {
      ...
      expect(((ast.right as FunctionNode).name as IdentifierNode).name).toBe('first');
    });
  ...
  describe('Complex Expressions', () => {
    it('should parse navigation with filtering', () => {
      const ast = parse('Patient.name.where(use = "official").given') as BinaryNode;
      expect(ast.type).toBe(NodeType.Binary);
      expect(ast.operator).toBe('.');
    ...
    it('should parse nested function calls', () => {
      ...
      expect((substring.arguments[1] as FunctionNode).type).toBe(NodeType.Function);
      expect(((substring.arguments[1] as FunctionNode).name as IdentifierNode).name).toBe('indexOf');
    });

    it('should parse complex boolean expressions', () => {
      const ast = parse('age >= 18 and status = "active" or priority = 1') as BinaryNode;
      expect(ast.type).toBe(NodeType.Binary);
      expect(ast.operator).toBe('or');
      expect((ast.left as BinaryNode).type).toBe(NodeType.Binary);
